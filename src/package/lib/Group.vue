<template>
  <div
    class="group"
    @touchstart.prevent="_start($event)"
    @touchmove.prevent="_move($event)"
...
export default {
    ...
    methods: {
        ...
        stop(diff) {
            ...
            // 移动到最接近的那一行
            this.translate =
                Math.round(this.translate / this.height) * this.height;
            const max = util.getMax(this.offset, this.height);
            const min = util.getMin(
                this.offset,
                this.height,
                this.items.length
            );
...
.group:before {
    ...
    background: linear-gradient(
        ...
        linear-gradient(0deg, hsla(0, 0%, 100%, 0.95), hsla(0, 0%, 100%, 0.5));
